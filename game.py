...
class Game:
    ...
    def process_move(self, move):
        ...
        for moves_in_col in self._current_moves:
            for move in moves_in_col:                # why not for move in self._current_moves?
                if move.label == self.current_player.label:
                    current_simple_moves.add((move.row, move.col))  # why not (row, col)?
    ...
    def is_tied(self):      
        ...
        return not self.has_winner and len([_ for (_, _, l) in flatten(self._current_moves) if not bool(l)]) == 0
